...
parser.add_argument('-o','--outputfile',nargs="?",
                    help='Supply a name and format for the output file')

#ugly variables
RX_SHARED_DATA = re.compile(r'window._sharedData = ({[^\n]*});')


def login(username,password,filename,user):
    print("Login...")
    ...
    if login.status_code != 200:
        ...
        if r.text.find("%s" % username) == -1:
            ...
            if user is not None:
                ...
                retrieve_pages(filename,session,target,page_name,section_name,base_url)
            else:
                print("Please select an option: profile or hastag")


def retrieve_pages(filename,session,target,page_name,section_name,base_url):
    ...
    for code in shortcodes:
        ...
        #if (real_day.month >=1 and real_day.month <=3 and real_day.year>=2018):
        #print(data["entry_data"]["PostPage"][0]["graphql"]["shortcode_media"]["edge_media_to_caption"]["edges"])
        try:
            caption = (data["entry_data"]["PostPage"][0]["graphql"]["shortcode_media"]["edge_media_to_caption"]["edges"][0]["node"]["text"].encode("utf-8"))

            database.write(caption)
        except:
            pass
...
if args.outputfile:
    ...
    outputfile = os.getcwd()+ "%s.csv" % user


login(username,passw,outputfile,user)
