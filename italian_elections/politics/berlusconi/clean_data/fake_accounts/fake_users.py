...
def retrieve_pages(database,connection,session,target,page_name,section_name,base_url):
    ...
    #print( data['entry_data'][page_name][0][section_name].keys())
    #print(data['entry_data'][page_name][0][section_name]["full_name"])
    #print(data['entry_data'][page_name][0][section_name]["biography"])
    #print(data['entry_data'][page_name][0][section_name]["is_verified"])
    #print(data['entry_data'][page_name][0][section_name]["username"])
    #print(data['entry_data'][page_name][0][section_name]["followed_by"]["count"])
    #print(data['entry_data'][page_name][0][section_name]["mutual_followers"]["usernames"])
    #print(data['entry_data'][page_name][0][section_name]["is_private"])
    #print(data['entry_data'][page_name][0][section_name]["connected_fb_page"])
    #print(data['entry_data'][page_name][0][section_name]["follows"]["count"])
    try:
        ...
        if ratio>=5.0:
            ...
            print(data.keys())
            sys.exit(-1)
            print(following,follower,data['entry_data'][page_name][0][section_name]["username"])
            username = data['entry_data'][page_name][0][section_name]["username"]
            biography = (data['entry_data'][page_name][0][section_name]["biography"])
            followers =   (data['entry_data'][page_name][0][section_name]["followed_by"]["count"])
            following =  (data['entry_data'][page_name][0][section_name]["follows"]["count"])
            ...
            for comm in comm_command:
                print(comm[0])
                conn_cursor.execute('''INSERT INTO fake_users(username,comment_text,biography,following,followers)
                  VALUES (?,?,?,?,?)''',(username,comm[0],biography,following,followers) )
                connection.commit()
